...
(function (root, factory) {
  ...
  sndcore = (function () {
    ...
    if (typeof window.AudioBufferSourceNode.prototype.start !== "function") {
      window.AudioBufferSourceNode.prototype.start =
        window.AudioBufferSourceNode.prototype.noteGrainOn;
    }
    if (typeof window.AudioBufferSourceNode.prototype.stop !== "function") {
      window.AudioBufferSourceNode.prototype.stop =
        window.AudioBufferSourceNode.prototype.noteOff;
    }
    if (typeof window.OscillatorNode.prototype.start !== "function") {
      window.OscillatorNode.prototype.start =
        window.OscillatorNode.prototype.noteOn;
    }
    if (typeof window.OscillatorNode.prototype.stop !== "function") {
      window.OscillatorNode.prototype.stop =
        window.OscillatorNode.prototype.noteOff;
    }
    if (
      !window.AudioContext.prototype.hasOwnProperty("createScriptProcessor")
    ) {
  ...
  oscillator = (function () {
    ...
    p5.Oscillator.prototype.phase = function (p) {
      ...
      var now = p5sound.audiocontext.currentTime;
      this.dNode.delayTime.linearRampToValueAtTime(
        p5.prototype.map(p, 0, 1, 0, 1 / this.oscillator.frequency.value),
        now
      );
    };
    // ========================== //
  ...
  env = (function () {
    ...
    p5.Env.prototype.play = function (unit, secondsFromNow) {
      ...
      this.control.linearRampToValueAtTime(
        this.dLevel,
        t + this.aTime + this.dTime
      );
      // hold sustain level
      this.control.linearRampToValueAtTime(
        this.sLevel,
        t + this.aTime + this.dTime + this.sTime
      );
      // release
      this.control.linearRampToValueAtTime(
        this.rLevel,
        t + this.aTime + this.dTime + this.sTime + this.rTime
      );
    ...
    p5.Env.prototype.triggerRelease = function (unit, secondsFromNow) {
      ...
      if (now - this.lastAttack < this.aTime) {
        ...
        this.control.linearRampToValueAtTime(
          this.sLevel,
          t + a + this.dTime + this.sTime
        );
        this.control.linearRampToValueAtTime(
          this.rLevel,
          t + a + this.dTime + this.sTime + this.rTime
        );
